...
function router(response, data) {
    ...
    if (opt === "latest") {
        // get latest movies
        movies.find({rating:{$gt:9.0}}).toArray((err, result)=>{
            response.write(JSON.stringify(result));
            response.end();
        });
    } else if (opt === "search") {
        movies.find({$or:[{'title': {$regex: data.keyword}},{'id': +data.id}]} ,{'limit': data.limit}).toArray((err, result) => {
            response.write(JSON.stringify(result));
            response.end();
        });
    } else if (opt === "category") {
        movies.find({'genres': {$regex: data.type}}, {'limit': data.limit}).toArray((err, result) => {
            response.write(JSON.stringify(result));
            response.end();
        });
    } else if (opt === "id") {
...
console.log('Server is running at http://127.0.0.1:1003/');